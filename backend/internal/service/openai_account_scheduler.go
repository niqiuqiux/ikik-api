...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if s.service.shouldClearStickySessionForSchedulingRequest(ctx, account, req.RequestedModel) ||
		!s.service.isOpenAIAccountEligibleForSchedulingRequest(ctx, account, req.RequestedModel, false, req.RequiredCapability) {
		_ = s.service.deleteStickySessionAccountID(ctx, req.GroupID, sessionHash)
		return nil, nil
	}
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	for i := range accounts {
		...
		if !s.service.isOpenAIAccountEligibleForSchedulingRequest(ctx, account, req.RequestedModel, false, req.RequiredCapability) {
			continue
		}
